...
Serveren kan per definisjon ikke fortelle deg at den er utilgjengelig. Er den nede, kommer det ingenting derfra å rendre. Derfor lager komponenten innholdet selv, og byggeren setter `data-ignore-morph` så en senere patch ikke river linja bort i det øyeblikket forbindelsen er tilbake. Se [Server først](/server-forst/).
