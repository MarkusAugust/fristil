...
### Og hvis serveren sender feltet på nytt

Oppdaterer serveren det samme området mens brukeren holder på, som i en Datastar- eller htmx-app, må malen i tillegg ta med lista over hva som skal bevares. Uten den fjernes koblingen komponenten satte, ved første oppdatering:
...
Hvert element trenger sin egen liste. Komponenten legger `fs-label` og
...
`id` står på hjelpeteksten og feilmeldingen selv om malen ikke skriver noen.
Komponenten lager dem i så fall selv, og `aria-describedby` på kontrollen
peker på dem. Forsvinner id-en i en oppdatering, peker koblingen på noe som
ikke finnes, og skjermleseren leser verken hjelpeteksten eller feilen.

Listene ligger i `FIELD_PRESERVED_ATTRIBUTES`, med én nøkkel per element
(`label`, `control`, `help` og `error`), så du slipper å skrive dem av.

Det gir to veier til den samme kontrakten:

| Hvem lager markupen | Sender serveren den på nytt? | Bruk |
| --- | --- | --- |
| Kode du skriver i JavaScript | uansett | `fs.field()`. Komponenten trengs ikke |
| En mal i et annet språk, eller HTML du skriver selv | nei | `<fs-field>` |
| En mal i et annet språk, eller HTML du skriver selv | ja | `<fs-field>` **og** `data-preserve-attr` |

De to spørsmålene er uavhengige, og skal ikke slås sammen. Hvem som lager
markupen avgjør om du bruker byggefunksjonen eller web componenten. Om
serveren kan sende den på nytt avgjør om `data-preserve-attr` trengs. En
statisk HTML-side bruker altså web componenten uten lista, og en React-app
uten server bruker byggefunksjonen.
