...
| Klasse | Hva det er |
| --- | --- |
| `.fs-select` | Nedtrekkslisten. Tilstanden kommer fra `data-state`. |

| Variabel | Standard |
| --- | --- |
| `--fs-select-radius` | `var(--size-1)` |

Pilen er to gradienter i bakgrunnen, tegnet i `--semantic-muted-foreground`, så den snur med temaet. Skal den bort, sett `background-image: none`.
