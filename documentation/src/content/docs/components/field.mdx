...
| --- | --- |
| **Ja.** React, Astro, en Node-server, eller en app helt uten server | [`fs.field()`](#som-funksjon). Da trenger du ikke elementet |
...
`<fs-field>` er det eneste unntaket i systemet, fordi koblingen er markup og ikke oppførsel. [Tabs](../tabs/), [Popover](../popover/), [Suggestion](../suggestion/) og [Error Summary](../error-summary/) gjør noe en funksjon ikke kan, som piltaster og fokus, så der bruker du byggeren **og** elementet. Se [Markup og oppførsel](/markup-og-oppforsel/#bygger-eller-element).
:::
...
Sender serveren det samme området på nytt mens brukeren holder på, som i en Datastar-app, må malen din ta med `data-preserve-attr` på de tre elementene. Ellers fjernes koblingen ved neste oppdatering. Lista ligger i `FIELD_PRESERVED_ATTRIBUTES`, og hele begrunnelsen står i [Markup og oppførsel](/markup-og-oppforsel/#når-markupen-ikke-lages-med-javascript).
