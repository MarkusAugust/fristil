...
<Eksempel importer={["tokens.css", "fieldset.css", "radio.css"]}
  stiler={["fieldset", "radio", "label"]} layout="stabel" maxWidth="28rem" kode={`
...
<fieldset {...fs.fieldset({ state: "invalid" })}>
  <legend {...fs.legend()}>Hvordan vil du ha svar?</legend>
</fieldset>
```
