...
```html
<fs-popover placement="bottom-end" data-preserve-attr="open">
  <button slot="trigger" class="fs-button" data-variant="secondary"
          aria-expanded="false" aria-controls="handlinger"
...
Knappen merkes med `slot="trigger"`. Alt annet innhold blir panelet.

Koblingen, `popover`-attributtet og klassen kommer fra serveren med `fs.popover()`. Komponenten setter bare to ting: `aria-expanded` når brukeren åpner, og posisjonen mot knappen. Begge står i `data-preserve-attr`, ellers river en oppdatering fra serveren dem bort. Se [Markup og oppførsel](/markup-og-oppforsel/).
...
<Tabs>
  <TabItem label="Ren HTML">
    ```html
    <fs-popover id="meny" placement="bottom-end" data-preserve-attr="open">
      <button slot="trigger" class="fs-button" data-variant="secondary"
              aria-expanded="false" aria-controls="handlinger"
              data-preserve-attr="aria-expanded">Handlinger</button>
