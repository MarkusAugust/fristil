...
| På `<dialog>` | Serveren når den vet, og nettleseren når `showModal()` kalles | Ja |
...
Serveren skriver det likevel når den vet at dialogen skal vises, og `fs.dialog({ open: true })` gir det på begge elementene. Det er av to grunner. Uten JavaScript er en `<dialog>` uten `open` skjult, så innholdet serveren ville vise finnes ikke for leseren; med `open` står det der som en boks på siden, og komponenten gjør den om til en ekte modal når den får kjøre. Og i React setter komponenten `open` før React hydrerer, så sto det ikke i serverens HTML, meldte React avvik ved hvert eneste oppslag.
...
Lager du markupen med JavaScript og styrer alt i nettleseren, trenger du ingen komponent: kall `showModal()` selv, som i «Komplett eksempel» over.
