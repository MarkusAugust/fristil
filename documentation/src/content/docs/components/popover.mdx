...
Koblingen, `popover`-attributtet og klassen kommer fra serveren med `fs.popover()`. Komponenten setter bare to ting: `aria-expanded` når brukeren åpner, og posisjonen mot knappen. Begge står i `data-preserve-attr`, ellers river en oppdatering fra serveren dem bort. Se [Server først](/server-forst/).
