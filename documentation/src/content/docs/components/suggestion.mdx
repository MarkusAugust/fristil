...
| `startsWith` framfor «inneholder» | Treff midt i ordet blir borte igjen |
| Uskarpt eller asynkront søk | Lista blinker mens de to er uenige |
...
Bygg da `fs.suggestion({ count: treff.length })`, ikke med hele lista: `count` avgjør om «Ingen treff» starter skjult.

**Antall treff leses opp uansett hvem som filtrerte,** og «Ingen treff» skjules og vises av komponenten som før. `prefiltered` slår av skjulingen av alternativer, ingenting annet. Komponenten tar også piltastene, markeringen og `aria-activedescendant`, altså det verken React eller serveren gjør.
