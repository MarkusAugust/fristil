...
Forhåndsvisningen viser dialogen slik serveren sender den: en `<dialog>` med `open`, som ennå ikke er gjort modal. En dialog er tatt ut av flyten, så her tvinges den tilbake i boksen for å kunne vises, og det er det eneste forhåndsvisningen legger til.
...
Markupen over er en boks på siden, ikke en modal. `showModal()` er det som flytter fokus inn, holder det der, lukker på Escape og maler flaten bak. Under står de to måtene å få det kallet gjort på.
