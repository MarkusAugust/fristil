...
**Oppgi `id` når feltet rendres to ganger.** Utelater du den, lager `fs.field()` en tilfeldig id, og to kjøringer gir to ulike. Rendrer serveren feltet og nettleseren det samme feltet på nytt, peker `for` og `aria-describedby` da på noe annet enn det som står der. I React er `useId()` laget for nettopp dette, og da melder den heller ikke avvik ved hydreringen.
...
<Tabs>
  ...
  <TabItem label="React">
    ...
    `fs.field()` lager en id når den ikke får en, og den kan ikke bli den samme på serveren og i nettleseren. React melder da avvik ved hydreringen på ledeteksten, feltet og hjelpeteksten, og koblingen mellom dem er brutt til React har rettet den opp.
