...
Pakken har én avhengighet, `lit`, som installeres sammen med den. Ramme- og frittstående komponenter er bygd på Lit, og den koster rundt 9,8 kB gzippet. Bruker du bare CSS-komponentene, havner Lit aldri i bunten din, siden ingenting registreres ved import alene. Tailwind-temaet er ren CSS, så pakken krever ingenting av Tailwind.
...
Pakken følger [semantisk versjonering](https://semver.org/lang/no/). Det offentlige API-et er større enn funksjonene: klassenavnene, `data-*`-attributtene, `part`-navnene, tokennavnene, `--fs-*`-variablene og oppføringene i `exports` er alle noe du kan bygge på, og de endres ikke uten et nytt hovedtall.

Hva som er endret i hver versjon står i [CHANGELOG.md](https://github.com/MarkusAugust/fristil/blob/master/designsystem/CHANGELOG.md), som også følger med i pakken.
...
- [Mønstre](../monster/skjema/): hele skjemaer, lister og sider satt sammen
