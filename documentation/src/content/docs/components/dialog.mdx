...
Fanen «Prøv den» er dialogen i drift. De fire andre er den samme dialogen skrevet for hvert sitt miljø, og hver av dem er hele oppskriften: markupen, koblingen og kallet. Stilarkene er de samme i alle fire, og står i hver fane.
...
<Tabs>
  ...
  <TabItem label="Ren HTML">
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@fristil/designsystem@0.14.0/src/components/ramme/dialog/dialog.css">
  ...
  <TabItem label="React">
    ...
    export function SlettSøknaden({ slett }: { slett: () => void }) {
      const dialog = useRef<HTMLDialogElement>(null)
      const boks = fs.dialog({ titleId: "bekreft-tittel" })
      ...
      return (
        <>
          ...
          <dialog
            ...
            ref={dialog}
            onClose={(e) => {
              if (e.currentTarget.returnValue === "slett") slett()
            }}
          >
  ...
  <TabItem label="Astro">
    ...
    <button {...fs.button({ variant: "danger" })} type="button" id="apne">
      Slett søknaden
    </button>

    <dialog {...boks.dialog} id="bekreft">
      <h2 {...boks.title}>Slette søknaden?</h2>
      <div {...boks.body}>
        <p>Søknaden og vedleggene blir borte. Dette kan ikke angres.</p>
      </div>
    ...
    <script>
      const dialog = document.getElementById("bekreft") as HTMLDialogElement
  ...
  <TabItem label="Datastar">
    ...
    ```html
    <!-- I sidemalen, én gang. Registreringen skjer ved oppstart, ikke ved hver
         patch, og den må stå utenfor området serveren sender på nytt.

         Adressen er en URL og ikke "@fristil/designsystem/dialog", fordi dette
         er sporet uten byggverktøy: en nettleser kan ikke slå opp et pakkenavn
         uten en importmap eller en bunter. -->

    <!-- Én gang for hele appen, i sidemalen. Ikke én gang per komponent. -->
    <script type="module">
      import { defineFsDialog } from
        "https://cdn.jsdelivr.net/npm/@fristil/designsystem@0.14.0/dist/components/ramme/dialog/fs-dialog.js"

      defineFsDialog()
    </script>

    <!-- Området serveren sender, og sender på nytt ved hver patch. Står
         `open` på verten, kaller komponenten showModal(). Ingen klientkode
         åpner noe. -->
    <fs-dialog id="bekreft" open>
      <dialog class="fs-dialog" aria-labelledby="bekreft-tittel" open>
        <h2 class="fs-dialog__title" id="bekreft-tittel">Slette søknaden?</h2>
        <div class="fs-dialog__body">
          <p>Søknaden og vedleggene blir borte. Dette kan ikke angres.</p>
        </div>
        <form method="dialog" class="fs-dialog__footer">
          <button class="fs-button" data-variant="secondary" value="avbryt"
                  data-on:click="@post('/avbryt')">Avbryt</button>
          <button class="fs-button" data-variant="danger" value="slett"
                  data-on:click="@post('/slett')">Slett søknaden</button>
        </form>
      </dialog>
    </fs-dialog>
    ```
...
## Det som er felles for alle fire

Å åpne dialogen **må** være et kall. `showModal()` flytter fokus inn, holder det der, lukker på Escape og gjør resten av siden utilgjengelig. `<dialog open>` alene gjør ingen av delene, og er bare en boks på siden. De tre første oppskriftene gjør kallet selv; Datastar-oppskriften lar `<fs-dialog>` gjøre det, fordi en server som bare sender HTML ikke har noe sted å kalle fra.
...
<Eksempel
  ...
  maxWidth="34rem"
  visningsCss={`
    /* Bare for å vise dialogen i boksen. En åpen <dialog> er
       position: absolute og ville falt ut av forhåndsvisningen. I en ekte
       side trengs ingenting av dette. */
    .fs-dialog { position: static; }
  `}
  kode={`
<dialog class="fs-dialog" aria-labelledby="slette-tittel" open>
  <h2 class="fs-dialog__title" id="slette-tittel">Slette søknaden?</h2>
  <div class="fs-dialog__body">
    <p>Søknaden og vedleggene blir borte. Dette kan ikke angres.</p>
  </div>
